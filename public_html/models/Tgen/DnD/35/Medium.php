<?php
class Tgen_DnD_35_Medium extends Tgen_DnD_35 {
    ...
    public function generate() {
        ...
        if ($d100 > 83) { // Wondrous Items
            $wondrous = new Tgen_DnD_35_Medium_Wondrous($this->_hoard);
            $this->_item = $wondrous->generate();
            unset($wondrous);
        } elseif ($d100 > 68) { // Wands
            $wands = new Tgen_DnD_35_Medium_Wand($this->_hoard);
            $this->_item = $wands->generate();
            unset($wands);
        } elseif ($d100 > 65) { // Staffs
            $staffs = new Tgen_DnD_35_Medium_Staff($this->_hoard);
            ...
            $rods = new Tgen_DnD_35_Medium_Rod($this->_hoard);
            $this->_item = $rods->generate();
            unset($rods);
        } elseif ($d100 > 30) { // Rings
            $ring = new Tgen_DnD_35_Medium_Ring($this->_hoard);
            $this->_item = $ring->generate();
            unset($ring);
        } elseif ($d100 > 20) { // Potions
            $potion = new Tgen_DnD_35_Medium_Potion($this->_hoard);
            $this->_item = $potion->generate();
            unset($potion);
        } elseif ($d100 > 10) { // Weapons
            $weapon = new Tgen_DnD_35_Medium_Weapon($this->_hoard);
            $this->_item = $weapon->generate();
            unset($weapon);
        } else { // Armor & Shields
            $armor = new Tgen_DnD_35_Medium_Armor($this->_hoard);
